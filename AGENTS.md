...
After editing Python files (like `ai_assistant_extension/routes.py`):

```bash
python -m py_compile ai_assistant_extension/__init__.py  # Check single file for syntax errors
```
...
**Python** (in `ai_assistant_extension/*.py` files):
...
- Backend processing in `ai_assistant_extension/routes.py`
...
- **✅ Do**: `ai_assistant_extension/` with underscores, all lowercase
...
```typescript
const PLUGIN_ID = 'ai_assistant_extension:plugin';
```
...
export namespace CommandIDs {
  export const openPanel = 'ai_assistant_extension:open-panel';
  export const refreshData = 'ai_assistant_extension:refresh-data';
}
...
const plugin: JupyterFrontEndPlugin<void> = {
  id: 'ai_assistant_extension:plugin',
  autoStart: true,
...
- Backend routes: `ai_assistant_extension/routes.py` (class `DataRouteHandler`)
- Frontend logic: `src/` directory
- Python package: `ai_assistant_extension/` directory
...
1. **Read the backend first** — Check `ai_assistant_extension/routes.py` to understand the existing API contract
...
Create RESTful endpoints in `ai_assistant_extension/routes.py`:
...
def setup_route_handlers(web_app):
    ...
    data_route = url_path_join(base_url, "ai_assistant_extension", "data")
...
- Define matching endpoint path strings (e.g., `"hello"`, `"get-data"`) in both `ai_assistant_extension/routes.py` and `src/api.ts` to ensure routes sync between backend and frontend
...
2. **Write the code** (TypeScript in `src/`, styles in `style/`, Python in `ai_assistant_extension/`)
...
- **Output**: Creates files in `lib/` and `ai_assistant_extension/labextension/`
...
**After editing Python** (files in `ai_assistant_extension/`):
...
- Ask user to search for the extension ID (`ai_assistant_extension`) to see if it loaded
...
ls -la ai_assistant_extension/labextension/  # Should contain bundled extension
```
...
- Backend routes: `ai_assistant_extension/routes.py`
- Server extension config: `jupyter-config/server-config/ai_assistant_extension.json`
...
**❌ Don't**: Write frontend interfaces without first checking the backend API contract in `ai_assistant_extension/routes.py`
...
```css
.jp-ai-assistant-extension-widget {
  padding: 8px;
}
```
...
**❌ Don't**: Use absolute imports like `from ai_assistant_extension.routes import ...`
...
- **Plugin ID** (in `src/index.ts`): `'ai_assistant_extension:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'ai_assistant_extension:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-ai-assistant-extension-ClassName`
- **API routes** (in `ai_assistant_extension/routes.py`): `url_path_join(base_url, "ai_assistant_extension", "endpoint")`
